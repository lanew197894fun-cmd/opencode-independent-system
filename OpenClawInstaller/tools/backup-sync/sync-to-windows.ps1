...
Write-Color "`n╔══════════════════════════════════════════════════════╗" Cyan
Write-Color "║           🔄 OpenClaw 同步工具 (PowerShell)        ║" Cyan
Write-Color "╚══════════════════════════════════════════════════════╝`n" Cyan
...
$Paths = @{
    ...
    # Windows 目標
    "OpenClawWin" = "C:\Users\ReaMasTer\.openclaw"
...
function Show-Paths {
    ...
    Write-Host "  Win OpenClaw: $($Paths.OpenClawWin)"
...
用法:
  ...
  .\sync.ps1 -OpenClaw             同步 .openclaw 配置
...
if ($All) {
    ...
    if (Test-Path $openclawSource) {
        Sync-Folder -From $openclawSource -To $Paths.OpenClawWin -Name "OpenClaw 配置"
    }
    ...
    Write-Color "==> 同步 OpenClaw 配置`n" Cyan
    $openclawSource = "$($Paths.WSLHome)\.openclaw"
    if (Test-Path $openclawSource) {
        Sync-Folder -From $openclawSource -To $Paths.OpenClawWin -Name "OpenClaw 配置"
    } else {
