...
test_whatsapp() {
    ...
    if check_openclaw_installed; then
        ...
        log_warn "WhatsApp 测试需要 OpenClaw 已安裝"
        echo -e "${YELLOW}請先完成 OpenClaw 安裝${NC}"
        return 1
    fi
}
...
test_imessage() {
    ...
    if check_openclaw_installed; then
        ...
        log_warn "iMessage 测试需要 OpenClaw 已安裝"
        echo -e "${YELLOW}請先完成 OpenClaw 安裝${NC}"
        return 1
    fi
}
...
run_openclaw_doctor() {
    ...
    if check_openclaw_installed; then
        ...
        log_error "OpenClaw 未安裝"
        echo -e "${YELLOW}請先运行 install.sh 安裝 OpenClaw${NC}"
        return 1
    fi
}
...
config_telegram() {
    ...
    if [ -n "$bot_token" ] && [ -n "$user_id" ]; then
        ...
        if check_openclaw_installed; then
            ...
            log_error "OpenClaw 未安裝，請先安裝 OpenClaw"
        fi
...
config_whatsapp() {
    ...
    if ! check_openclaw_installed; then
        log_error "OpenClaw 未安裝，請先运行安裝脚本"
        press_enter
        return
    fi
...
config_feishu_app() {
    ...
    echo ""
    echo -e "${YELLOW}⚠️ 請确保 OpenClaw Gateway 服务已啟動${NC}"
    echo ""
...
EOF
    ...
    if check_openclaw_installed; then
        ...
        if [ -n "$openclaw_model" ]; then
            ...
            log_info "OpenClaw 預設模型已設定为: $openclaw_model"
        fi
    fi
