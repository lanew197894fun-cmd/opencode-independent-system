...
import { JiSanJiSystem } from "./ji-san-ji-system"

async function main() {
  console.clear()
  console.log("╔═══════════════════════════════════════════════════════════╗")
  console.log("║           計散機架構 (Ji San Ji Framework) v1.0.0         ║")
  ...
  try {
    console.log("📦 初始化系統模組...")
    const jiSanJi = new JiSanJiSystem()
    console.log("✅ 系統初始化完成\n")
    ...
    const prompt = () => {
      rl.question("\n🤖 請輸入訊息: ", async (input) => {
        ...
        if (cmd === ":exit" || cmd === "exit" || cmd === "q") {
          console.log("\n🛑 正在關閉系統...")
          await jiSanJi.shutdown()
          console.log("✅ 系統已關閉")
          rl.close()
          process.exit(0)
        } else if (cmd === ":health") {
          console.log("\n🚀 執行健康檢查...")
          const h = await jiSanJi.healthCheck()
          ...
          console.log("\n⏳ 處理中...")
          const result = await jiSanJi.process(input)
