...
const log = Log.create({ service: "ji-san-ji-system" });
...
export class JiSanJiSystem {
  ...
  async process(
    ...
    try {
      ...
      return {
        ...
        steps,
        knowledgeUsed,
        toolsUsed: ["knowledge", "search", "problem-solver", "memory"],
        timeTaken: Date.now() - startTime,
  ...
  private async generateResponse(
    ...
    if (
      !this.offlineMode &&
      !options.offline &&
      this.modelManager.isConnected()
    ) {
      try {
        const result = await this.modelManager.chat([
