...
#
# ═══════════════════════════════════════════════════════════════════════════
# ║  🦞 OpenCode + OpenCode 整合啟動器 v2.0                               ║
# ║  一鍵啟動 • 狀態監控 • 資源管理 • 自動修復                              ║
# ═══════════════════════════════════════════════════════════════════════════
#
# 說明：
#   這是 OpenCode 的主要啟動腳本，提供服務管理、狀態監控、備份等功能。
#
# 使用方法：
...
#
# 環境變數：
#   CONFIG_DIR    - OpenCode 配置目錄 (預設: ~/.openclaw)
...
show_status() {
    ...
    if is_gateway_running; then
        local pid=$(get_gateway_pid)
        echo -e "║  OpenCode Gateway  ${GREEN}運行中${NC}    $pid"
    else
        echo -e "║  OpenCode Gateway  ${RED}停止${NC}        -"
    fi
...
do_start() {
    echo ""
    log_info "啟動 OpenCode Gateway..."
    openclaw gateway start
    sleep 2
    
    if is_gateway_running; then
        log_info "OpenCode Gateway 啟動成功"
    else
        log_error "OpenCode Gateway 啟動失敗"
        return 1
    fi
}
...
# 停止
do_stop() {
    log_info "停止 OpenCode Gateway..."
    openclaw gateway stop
    sleep 2
    
    if ! is_gateway_running; then
        log_info "OpenCode Gateway 已停止"
    else
        log_warn "強制停止..."
        local pid=$(get_gateway_pid)
...
do_health() {
    ...
    # Gateway
    if is_gateway_running; then
        echo -e "  ${GREEN}✓${NC} OpenCode Gateway 運行中"
    else
        echo -e "  ${RED}✗${NC} OpenCode Gateway 未運行"
        issues=$((issues + 1))
    fi
...
do_backup_external() {
    ...
    if [ -z "$target" ]; then
        ...
        $HOME/.opencode/user-scripts/backup-external.sh sync "$target" "OpenCode"
    fi
}
...
case "${1:-menu}" in
    ...
    help|--help|-h)
        ...
        echo ""
        echo "命令:"
        echo "  start     啟動 OpenCode Gateway"
        echo "  stop      停止 OpenCode Gateway"
        echo "  restart   重啟 OpenCode Gateway"
        echo "  status    顯示系統狀態"
        echo "  health    健康檢查"
