...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                                                                           ║
# ║   🦞 OpenCode 交互式配置菜单 v1.0.0                                        ║
# ║   便捷的可视化配置工具                                                      ║
# ║                                                                           ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
#
# 說明：
#   這是 OpenCode 的互動式配置選單，提供圖形化介面進行各種設定。
#   支援終端機和管道模式執行。
#
# 使用方法：
...
try:
    ...
    # 确保 channels.xxx 存在（使用安全的預設策略，不設定 enabled）
...
restart_gateway_for_channel() {
    ...
    if [ -n "$gateway_pid" ]; then
        ...
        if [ -n "$dashboard_url" ]; then
            ...
            echo ""
            echo -e "${YELLOW}⚠️  請使用此 URL 访问控制界面，否则会提示 token_missing${NC}"
        else
            echo ""
        ...
        echo "  • 运行 ${WHITE}openclaw gateway start${NC} 手動啟動"
    fi
}

# 检查 OpenCode Gateway 是否运行
...
test_ai_connection() {
    ...
    if [ $exit_code -eq 0 ] && ! echo "$result" | grep -qiE "error|failed|401|403|Unknown model"; then
        ...
        if echo "$result" | grep -q "Unknown model"; then
            echo -e "${YELLOW}提示: 模型不被 OpenCode 识别${NC}"
...
test_discord_bot() {
    ...
    if [ "$guild_count" = "0" ] || [ -z "$guild_count" ]; then
        ...
        echo ""
        echo -e "${YELLOW}請先邀請机器人到你的服务器:${NC}"
        ...
        echo "  4. 复制链接并在浏览器中打开，選擇服务器"
        echo ""
        echo -e "${WHITE}邀請链接示例:${NC}"
...
" 2>/dev/null
    ...
    if echo "$channel_info" | grep -q '"id"'; then
        ...
        if echo "$error" | grep -qi "Unknown Channel"; then
            echo -e "${YELLOW}频道 ID 可能不正确，請重新复制${NC}"
        else
            echo -e "${YELLOW}机器人可能不在该频道所在的服务器中${NC}"
            echo "  請确保机器人已被邀請到正确的服务器"
        fi
        return 1
    fi
    ...
    local message="🦞 **OpenCode 测试消息** - 配置成功！时间: $timestamp"
    ...
    if echo "$send_result" | grep -q '"id"'; then
        log_info "测试消息发送成功！請检查 Discord 频道"
        return 0
    else
        ...
        if echo "$error" | grep -qi "Missing Access"; then
            ...
            echo "  1. 机器人未被邀請到该频道所在的服务器"
            echo "  2. 机器人在该频道没有发送消息权限"
            echo "  3. 频道 ID 不正确"
            echo ""
            echo -e "${WHITE}解决方法:${NC}"
            echo "  1. 确认机器人已被邀請到服务器:"
            echo "     • 重新生成邀請链接并邀請机器人"
            ...
            echo -e "${YELLOW}提示: 频道 ID 无效，請检查是否正确复制${NC}"
...
test_feishu_bot() {
    ...
    if [ "$code" != "0" ]; then
        ...
        log_error "获取 Token 失敗: $msg"
        echo ""
        echo -e "${YELLOW}請检查:${NC}"
...
" 2>/dev/null)
        ...
        echo -e "${GRAY}請求体: $request_body${NC}"
        ...
        echo ""
        echo -e "${YELLOW}如需发送测试消息，請提供群组 Chat ID${NC}"
        echo -e "${GRAY}获取方式: 群設定 → 群資訊 → 群号${NC}"
    fi
...
test_ollama_connection() {
    ...
    if [ -z "$health" ]; then
        log_error "无法連線到 Ollama 服务: $base_url"
        echo -e "${YELLOW}請确保 Ollama 正在运行: ollama serve${NC}"
        return 1
    fi
...
test_whatsapp() {
    ...
    if check_openclaw_installed; then
        ...
        log_warn "WhatsApp 测试需要 OpenCode 已安裝"
        echo -e "${YELLOW}請先完成 OpenCode 安裝${NC}"
        return 1
    fi
}
...
test_imessage() {
    ...
    if check_openclaw_installed; then
        ...
        log_warn "iMessage 测试需要 OpenCode 已安裝"
        echo -e "${YELLOW}請先完成 OpenCode 安裝${NC}"
        return 1
    fi
}
...
test_wechat() {
    ...
    if check_openclaw_installed; then
        ...
        log_warn "微信测试需要 OpenCode 已安裝"
        echo -e "${YELLOW}請先完成 OpenCode 安裝${NC}"
        return 1
    fi
}

# 运行 OpenCode 诊断 (使用 openclaw doctor)
run_openclaw_doctor() {
    echo ""
    echo -e "${CYAN}━━━ OpenCode 诊断 ━━━${NC}"
    echo ""
    ...
    if check_openclaw_installed; then
        ...
        log_error "OpenCode 未安裝"
        echo -e "${YELLOW}請先运行 install.sh 安裝 OpenCode${NC}"
        return 1
    fi
}

# 运行 OpenCode 状态检查 (使用 openclaw status)
run_openclaw_status() {
    echo ""
    echo -e "${CYAN}━━━ OpenCode 状态 ━━━${NC}"
    echo ""
...
# 运行 OpenCode 健康检查 (使用 openclaw health)
run_openclaw_health() {
    echo ""
...
show_status() {
    ...
    if command -v openclaw &> /dev/null; then
        echo -e "  ${GREEN}✓${NC} OpenCode 已安裝: $(openclaw --version 2>/dev/null || echo 'unknown')"
    ...
    if [ -f "$OPENCLAW_ENV" ]; then
        ...
        if check_openclaw_installed; then
            ...
            echo -e "    • 預設模型: ${WHITE}$default_model${NC}"
        fi
    ...
    [ -f "$OPENCLAW_JSON" ] && echo -e "    ${GREEN}✓${NC} OpenCode 配置: $OPENCLAW_JSON" || echo -e "    ${YELLOW}⚠${NC} OpenCode 配置"
...
config_ai_model() {
    ...
    case $choice in
        ...
        0) return ;;
        *) log_error "无效選擇"; press_enter; config_ai_model ;;
    esac
}
...
config_anthropic() {
    ...
    # 询问配置模式
    echo -e "${YELLOW}選擇配置模式:${NC}"
    print_menu_item "1" "仅更改模型 (保留当前 API Key 和地址)" "🔄"
    print_menu_item "2" "完整配置 (可修改所有設定)" "⚙️"
    echo ""
    read -p "$(echo -e "${YELLOW}請選擇 [1-2] (預設: 1): ${NC}")" config_mode < "$TTY_INPUT"
    ...
    # 验证 API Key
    if [ -z "$api_key" ]; then
        log_error "API Key 不能为空，請先配置 API Key"
        press_enter
        return
    fi
    ...
    read -p "$(echo -e "${YELLOW}請選擇 [1-5] (預設: 1): ${NC}")" model_choice < "$TTY_INPUT"
...
config_openai() {
    ...
    read -p "$(echo -e "${YELLOW}請選擇 [1-6] (預設: 1): ${NC}")" model_choice < "$TTY_INPUT"
    model_choice=${model_choice:-1}
    ...
    if [ -n "$base_url" ]; then
        ...
        echo ""
        read -p "$(echo -e "${YELLOW}選擇 API 格式 [1-2] (預設: 2): ${NC}")" api_type_choice < "$TTY_INPUT"
...
config_deepseek() {
    ...
    print_menu_item "3" "deepseek-coder (程式碼)" "💻"
    print_menu_item "4" "自定义模型名称" "✏️"
    echo ""
    
    read -p "$(echo -e "${YELLOW}請選擇 [1-4] (預設: 1): ${NC}")" model_choice < "$TTY_INPUT"
    model_choice=${model_choice:-1}
...
config_ollama() {
    ...
    if [ -n "$current_url" ]; then
        ...
        echo -e "  服务地址: ${GRAY}(使用預設)${NC}"
    fi
    echo ""
    
    echo -e "${CYAN}預設地址: ${WHITE}$default_url${NC}"
    echo ""
    print_divider
    echo ""
    ...
    if [ "$config_mode" = "2" ]; then
        ...
        [ -n "$current_url" ] && echo -e "  当前地址: ${WHITE}$current_url${NC}"
        echo -e "  預設地址: ${WHITE}$default_url${NC}"
        echo ""
    ...
    echo -e "${CYAN}選擇模型:${NC}"
    echo ""
    print_menu_item "1" "Qwen2.5-Coder (3B, 推薦)" "🧠"
    print_menu_item "2" "Llama 3 (8B)" "🦙"
    print_menu_item "3" "Llama 3 (70B)" "🦙"
    print_menu_item "4" "Mistral" "🌬️"
    print_menu_item "5" "CodeLlama" "💻"
    print_menu_item "6" "自訂模型名稱" "✏️"
    echo ""
    ...
    case $model_choice in
        ...
        5) model="codellama" ;;
        6) 
            read -p "$(echo -e "${YELLOW}輸入模型名稱: ${NC}")" model < "$TTY_INPUT"
            ;;
    ...
    # 询问是否测试
    echo ""
    if confirm "是否测试 Ollama 連線？" "y"; then
...
config_openrouter() {
    ...
    if [ "$config_mode" = "2" ]; then
        ...
        echo -e "${CYAN}API 地址配置:${NC}"
        [ -n "$current_url" ] && echo -e "  当前地址: ${WHITE}$current_url${NC}"
        echo -e "  預設地址: ${WHITE}$official_url${NC}"
        echo ""
        read -p "$(echo -e "${YELLOW}輸入 API 地址 (留空保持当前配置): ${NC}")" input_url < "$TTY_INPUT"
...
config_azure_openai() {
    ...
    read -p "$(echo -e "${YELLOW}API 版本 (預設: 2024-02-15-preview): ${NC}")" api_version
...
config_minimax() {
    ...
    echo -e "${YELLOW}選擇区域:${NC}"
    print_menu_item "1" "国际版 (minimax)" "🌍"
    print_menu_item "2" "国内版 (minimax-cn)" "🇨🇳"
    echo ""
    
    read -p "$(echo -e "${YELLOW}請選擇 [1-2] (預設: 1): ${NC}")" region_choice < "$TTY_INPUT"
    ...
    read -p "$(echo -e "${YELLOW}請選擇 [1-3] (預設: 1): ${NC}")" model_choice < "$TTY_INPUT"
    model_choice=${model_choice:-1}
...
config_opencode() {
    ...
    print_menu_item "1" "claude-sonnet-4-5 (推薦)" "⭐"
    print_menu_item "2" "claude-opus-4-5 (最强)" "👑"
    print_menu_item "3" "gpt-4o (GPT-5)" "🟢"
    print_menu_item "4" "gemini-3-pro (Gemini 3)" "🔴"
    print_menu_item "5" "glm-4.7-free (免费)" "🆓"
    print_menu_item "6" "gpt-4o-codex (程式碼)" "💻"
    print_menu_item "7" "自定义模型名称" "✏️"
    echo ""
    
    read -p "$(echo -e "${YELLOW}請選擇 [1-7] (預設: 1): ${NC}")" model_choice < "$TTY_INPUT"
    model_choice=${model_choice:-1}
...
config_google_antigravity() {
    ...
    echo -e "${GRAY}获取 API Key: 請联系 Google Cloud 获取访问权限${NC}"
    echo ""
    print_divider
    echo ""
...
config_channels() {
    ...
    case $choice in
        ...
        7) config_feishu ;;
        0) return ;;
        *) log_error "无效選擇"; press_enter; config_channels ;;
    esac
}
...
config_telegram() {
    ...
    echo "  2. 发送 /newbot 建立新机器人"
    echo "  3. 按提示設定名称，获取 Bot Token"
    ...
    if [ -n "$bot_token" ] && [ -n "$user_id" ]; then
        ...
        if check_openclaw_installed; then
            echo ""
            log_info "正在配置 OpenCode Telegram 渠道..."
            ...
            if openclaw channels add --channel telegram --token "$bot_token" 2>/dev/null; then
                ...
                log_warn "Telegram 渠道可能已存在或配置失敗"
            fi
            ...
            log_error "OpenCode 未安裝，請先安裝 OpenCode"
        fi
...
config_discord() {
    ...
    echo -e "${CYAN}第一步: 建立 Discord 应用和机器人${NC}"
    echo ""
    echo "  1. 访问 ${WHITE}https://discord.com/developers/applications${NC}"
    echo "  2. 点击 ${WHITE}New Application${NC} 建立新应用"
    ...
    echo "  4. 复制生成的 URL，在浏览器打开并選擇服务器"
    echo "  5. ${YELLOW}确保机器人在目标频道有权限！${NC}"
    echo ""
    echo -e "${CYAN}第三步: 获取频道 ID${NC}"
    echo ""
    echo "  1. 打开 Discord 客户端，进入 ${WHITE}用户設定 → 高级${NC}"
    ...
    if [ -n "$bot_token" ] && [ -n "$channel_id" ]; then
        ...
        if check_openclaw_installed; then
            echo ""
            log_info "正在配置 OpenCode Discord 渠道..."
            ...
            if openclaw channels add --channel discord --token "$bot_token" 2>/dev/null; then
                ...
                log_warn "Discord 渠道可能已存在或配置失敗"
            fi
            
            # 設定 groupPolicy 为 open（只响应 @ 机器人的消息）
            echo -e "${YELLOW}設定消息响应策略...${NC}"
            openclaw config set channels.discord.groupPolicy open 2>/dev/null || true
            log_info "已設定为: 响应 @机器人 的消息"
...
config_whatsapp() {
    ...
    if ! check_openclaw_installed; then
        log_error "OpenCode 未安裝，請先运行安裝脚本"
        press_enter
        return
    fi
    ...
    if confirm "是否继续？"; then
        ...
        echo -e "${YELLOW}請扫描顯示的二维码完成登录${NC}"
        echo ""
...
config_slack() {
    ...
    if [ -n "$bot_token" ] && [ -n "$app_token" ]; then
        ...
        if check_openclaw_installed; then
            echo ""
            log_info "正在配置 OpenCode Slack 渠道..."
        ...
        # 询问是否测试
        echo ""
        if confirm "是否验证 Slack 連線？" "y"; then
...
config_wechat() {
    ...
    echo "  • OpenCode 可能通过插件支援微信"
    echo "  • 請查看 OpenCode 文档了解详情"
    echo ""
...
config_imessage() {
    ...
    echo ""
    echo -e "${YELLOW}系統偏好設定 → 隐私与安全性 → 完整磁盘访问权限 → 添加终端${NC}"
    echo ""
...
config_feishu_app() {
    ...
    echo -e "${GREEN}✓ 个人账号即可使用，无需企业认证！${NC}"
    echo -e "${CYAN}  （"自建应用"只是飞书的命名，任何人都可以建立）${NC}"
    echo ""
    ...
    echo ""
    echo "  ${WHITE}第二步: 飞书开放平台建立应用${NC}"
    echo "    1. 访问 https://open.feishu.cn/"
    echo "    2. 建立企业自建应用 → 添加「机器人」能力"
    ...
    # ========== 第二、三步提示 ==========
    echo -e "${WHITE}━━━ 第二、三步: 請在飞书开放平台完成 ━━━${NC}"
    echo ""
    echo -e "${CYAN}請打开飞书开放平台完成以下操作:${NC}"
    echo "  1. 访问 https://open.feishu.cn/"
    echo "  2. 建立企业自建应用 → 添加「机器人」能力"
    echo "  3. 获取 App ID 和 App Secret"
    echo "  4. 权限管理 → 添加权限:"
    ...
    echo "     - im:chat:readonly (读取群資訊)"
    echo ""
    echo -e "${GREEN}💡 提示: 使用长連線模式，无需配置公网 Webhook 地址${NC}"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}📝 使用长連線模式，只需要 App ID 和 App Secret${NC}"
    ...
    echo -e "連線模式: ${WHITE}WebSocket 长連線${NC}"
    echo -e "${GREEN}✓ 无需公网服务器${NC}"
    echo ""
    echo -e "${YELLOW}⚠️  重要: 需要先啟動 Gateway 服务！${NC}"
    echo -e "${CYAN}   啟動后才能在飞书后台配置长連線${NC}"
    echo ""
    
    if confirm "是否现在啟動/重启 Gateway？" "y"; then
        restart_gateway_for_channel
    fi
    ...
    echo ""
    echo -e "${YELLOW}⚠️ 請确保 OpenCode Gateway 服务已啟動${NC}"
    echo ""
    echo -e "${CYAN}📋 在飞书开放平台完成以下配置:${NC}"
    echo ""
    echo -e "  ${WHITE}1. 事件与回调 → 選擇「使用长連線接收事件」${NC}"
    echo -e "     ${GREEN}✓ 无需公网服务器，无需 Webhook 地址${NC}"
    echo -e "     ${YELLOW}⚠️ 如果无法保存，請确认 Gateway 已啟動${NC}"
    echo ""
    ...
    if confirm "是否发送测试消息验证配置？" "y"; then
        echo ""
        echo -e "${CYAN}如需发送测试消息，請輸入群组 Chat ID:${NC}"
        echo -e "${GRAY}获取方式: 群設定 → 群資訊 → 群号${NC}"
        echo ""
...
config_security() {
    ...
    echo -e "${RED}⚠️ 警告: 以下設定涉及安全风险，請谨慎配置${NC}"
    echo ""
    ...
    echo -en "${YELLOW}請選擇 [0-5]: ${NC}"
    read choice < "$TTY_INPUT"
    
    case $choice in
        1)
            if confirm "允许 OpenCode 执行系統命令？这可能带来安全风险" "n"; then
                log_info "已启用系統命令执行"
            else
                log_info "已禁用系統命令执行"
            fi
            ;;
        2)
            if confirm "允许 OpenCode 读写檔案？" "n"; then
                log_info "已启用檔案访问"
            else
                log_info "已禁用檔案访问"
            fi
            ;;
        3)
            if confirm "允许 OpenCode 浏览网络？" "y"; then
                log_info "已启用网络浏览"
            else
                log_info "已禁用网络浏览"
            fi
...
manage_service() {
    ...
    echo ""
    echo -e "  ${RED}[8]${NC} 🗑️  卸载 OpenCode"
    echo ""
    print_menu_item "0" "返回主菜单" "↩️"
    echo ""
    
    echo -en "${YELLOW}請選擇 [0-8]: ${NC}"
    read choice < "$TTY_INPUT"
    ...
    case $choice in
        1)
            ...
            if command -v openclaw &> /dev/null; then
                ...
                if [ -n "$port_pid" ]; then
                    echo -e "${YELLOW}偵測到端口 $port 被其他进程占用 (PID: $port_pid)${NC}"
                ...
                if echo "$config_check" | grep -qi "Config invalid"; then
                    ...
                    echo ""
                    echo -e "${CYAN}請手動运行: openclaw doctor --fix${NC}"
                    press_enter
                    manage_service
                    return
                fi
                ...
                # 最终偵測：只要端口有服务就是成功（无论是刚啟動的还是之前已运行的）
                if [ -n "$gateway_pid" ]; then
                    ...
                    if [ -n "$dashboard_url" ]; then
                        ...
                        echo -e "  ${WHITE}$dashboard_url${NC}"
                        echo ""
                        echo -e "${YELLOW}⚠️  請使用此 URL 访问控制界面${NC}"
                    else
                        echo ""
                        echo -e "${YELLOW}提示: 运行以下命令获取带 token 的 Dashboard URL:${NC}"
                        echo -e "  ${WHITE}openclaw dashboard${NC}"
                    ...
                    echo -e "  2. 运行 ${WHITE}openclaw gateway${NC} 手動啟動查看详细错误"
                fi
            else
                log_error "OpenCode 未安裝"
            fi
        ...
        8)
            echo ""
            echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo -e "${RED}           ⚠️  卸载 OpenCode${NC}"
            echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            echo -e "${YELLOW}此操作将:${NC}"
            echo "  1. 停止 OpenCode 服务"
            echo "  2. 卸载 openclaw npm 包"
            echo "  3. 可选删除配置目錄 (~/.openclaw)"
            echo ""
            
            if ! confirm "确定要卸载 OpenCode 吗？" "n"; then
                log_info "已取消卸载"
                press_enter
                manage_service
                return
            ...
            if ! command -v openclaw &> /dev/null; then
                log_info "OpenCode 已卸载"
            else
                log_warn "卸载可能未完全成功，請手動运行: npm uninstall -g openclaw"
            fi
            
            # 4. 询问是否删除配置
            echo ""
            if [ -d "$HOME/.openclaw" ]; then
                echo -e "${YELLOW}偵測到配置目錄: ~/.openclaw${NC}"
                echo ""
            ...
            echo ""
            echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo -e "${GREEN}           ✓ OpenCode 卸载完成${NC}"
            echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            echo -e "${CYAN}如需重新安裝，請运行:${NC}"
            echo "  curl -fsSL https://raw.githubusercontent.com/miaoxworld/OpenCodeInstaller/main/install.sh | bash"
            echo ""
...
ensure_openclaw_init() {
    ...
    # 检查 gateway.auth 配置，如果是 token 模式但没有 token，则自動生成
...
EOF
    ...
    if check_openclaw_installed; then
        ...
        if [ -n "$openclaw_model" ]; then
            ...
            log_info "OpenCode 預設模型已設定为: $openclaw_model"
        fi
    fi
...
configure_custom_provider() {
    ...
    local base_url="$4"
    local config_file="$5"
    local custom_api_type="$6"  # 可选参数，用于覆盖預設 API 类型
    ...
    # 确定 API 类型
    # OpenCode 支援: anthropic-messages, openai-responses, openai-completions
...
" 2>/dev/null
            ...
            echo -e "${GRAY}(清理可避免配置累积，推薦選擇 Y)${NC}"
...
" 2>&1
        ...
        if [ $node_exit -eq 0 ]; then
            ...
            log_warn "node 配置失敗 (exit: $node_exit)，尝试使用 python3..."
        fi
    fi
    
    # 如果 node 失敗或不存在，尝试 python3
...
" 2>&1
        ...
        if [ $py_exit -eq 0 ]; then
            ...
            log_warn "python3 配置失敗 (exit: $py_exit)"
        fi
    fi
    ...
    if [ -f "$config_file" ]; then
        if grep -q "$provider_id" "$config_file" 2>/dev/null; then
            ...
            log_warn "配置檔案可能未正确写入，請检查: $config_file"
        fi
    fi
}
...
advanced_settings() {
    ...
    print_menu_item "6" "更新 OpenCode" "⬆️"
    print_menu_item "7" "卸载 OpenCode" "🗑️"
    print_menu_item "0" "返回主菜单" "↩️"
    echo ""
    ...
    case $choice in
        1)
            ...
            if [ -f "$OPENCLAW_ENV" ]; then
                ...
                log_error "环境变量檔案不存在: $OPENCLAW_ENV"
            fi
            ;;
        2)
            echo ""
        ...
        4)
            if confirm "确定要重置所有配置吗？这将删除当前配置" "n"; then
                ...
                log_info "配置已重置，請重新运行安裝脚本"
            fi
            ;;
        5)
            if confirm "确定要清理日志吗？" "n"; then
                if command -v openclaw &> /dev/null; then
                    openclaw logs clear 2>/dev/null || log_warn "OpenCode 日志清理命令不可用"
                fi
...
restore_config() {
    ...
    echo ""
    read -p "$(echo -e "${YELLOW}選擇要恢复的备份 [1-$((i-1))]: ${NC}")" choice
...
quick_test_menu() {
    ...
    if check_openclaw_installed; then
        ...
        echo -e "  ${GREEN}✓${NC} OpenCode 已安裝: $version"
    else
        echo -e "  ${YELLOW}⚠${NC} OpenCode 未安裝"
    fi
    echo ""
    print_divider
    echo ""
    
    echo -e "${CYAN}API 連線测试:${NC}"
    print_menu_item "1" "测试 AI API 連線" "🤖"
    ...
    echo ""
    echo -e "${CYAN}OpenCode 诊断 (需要已安裝):${NC}"
    ...
    case $choice in
        ...
        a|A) run_all_tests ;;
        0) return ;;
        *) log_error "无效選擇"; press_enter; quick_test_menu ;;
    esac
}
...
quick_test_ai() {
    ...
    echo -e "${WHITE}🤖 测试 AI API 連線${NC}"
    print_divider
    echo ""
    
    # 从环境变量檔案读取配置
    if [ ! -f "$OPENCLAW_ENV" ]; then
        log_error "AI 模型尚未配置，請先完成配置"
        press_enter
        quick_test_menu
        return
    fi
...
quick_test_telegram() {
    ...
    echo -e "${CYAN}請輸入 Telegram Bot Token 和 User ID 进行测试:${NC}"
    echo ""
...
quick_test_discord() {
    ...
    echo -e "${CYAN}請輸入 Discord Bot Token 和 Channel ID 进行测试:${NC}"
    echo ""
...
quick_test_slack() {
    ...
    echo -e "${CYAN}請輸入 Slack Bot Token 进行测试:${NC}"
    echo ""
...
" 2>/dev/null)
    ...
    if [ -n "$app_id" ] && [ -n "$app_secret" ]; then
        echo -e "${GREEN}✓ 偵測到已配置的飞书应用${NC}"
        echo -e "  App ID: ${WHITE}${app_id:0:15}...${NC}"
        echo ""
    else
        echo -e "${YELLOW}未偵測到飞书配置，請手動輸入:${NC}"
        echo ""
    ...
    echo ""
    echo -e "${CYAN}如需发送测试消息，請輸入群组 Chat ID（留空跳过）:${NC}"
    echo -e "${GRAY}获取方式: 群設定 → 群資訊 → 群号${NC}"
    echo ""
...
quick_test_ollama() {
    ...
    read -p "$(echo -e "${YELLOW}Ollama 地址 (預設: $base_url): ${NC}")" input_url
    [ -n "$input_url" ] && base_url="$input_url"
    
    read -p "$(echo -e "${YELLOW}模型名称 (預設: $model): ${NC}")" input_model
...
run_all_tests() {
    ...
    if [ -n "$provider" ] && [ -n "$api_key" ] && [ "$api_key" != "your-api-key-here" ]; then
        ...
        if [ "$http_code" = "200" ]; then
            ...
            log_error "AI API 测试失敗 (HTTP $http_code)"
        fi
        echo ""
    fi
    ...
    echo ""
    echo -e "${CYAN}渠道测试:${NC}"
    echo -e "  使用 ${WHITE}快速测试${NC} 菜单手動测试各个渠道"
    ...
    if [ $passed_tests -eq $total_tests ] && [ $total_tests -gt 0 ]; then
        ...
        echo ""
        echo -e "${YELLOW}⚠ 没有可测试的配置，請先完成相关配置${NC}"
    fi
    
    # 如果 OpenCode 已安裝，提示可用的诊断命令
    if check_openclaw_installed; then
        echo ""
...
show_main_menu() {
    ...
    print_menu_item "5" "安全設定" "🔒"
    print_menu_item "6" "服务管理" "⚡"
    print_menu_item "7" "快速测试" "🧪"
    print_menu_item "8" "高级設定" "🔧"
