# ═══════════════════════════════════════════════════════════════════════
# ║  🦞 OpenCode + OpenCode 整合啟動器 v2.0 (PowerShell)            ║
# ║  一鍵啟動 • 狀態監控 • 資源管理 • 自動修復                        ║
# ═══════════════════════════════════════════════════════════════════════
#
# 說明：
#   這是 OpenCode 的 Windows PowerShell 啟動腳本，提供服務管理與狀態監控。
...
#
# 功能：
#   1. 啟動/停止/重啟 OpenCode Gateway
#   2. 查看服務狀態與 PID
#   3. 監控資源使用 (CPU/記憶體)
#   4. 互動式選單介面
#
# 環境：
#   $env:USERPROFILE\.openclaw - OpenCode 配置目錄
...
function Show-Status {
    ...
    Write-Host ("║  OpenCode Gateway   {0,-11} {1,-15}║" -f $status, $pidStr)
    Write-Host "╚══════════════════════════════════════════════════════╝"
...
function Show-Menu {
    ...
    Write-Host ""
    Write-Host "╔══════════════════════════════════════════════════════╗"
    Write-Host "║     🦞 OpenCode 整合啟動器 v2.0                   ║"
    Write-Host "╠══════════════════════════════════════════════════════╣"
    Write-Host "║  1. 啟動服務                                        ║"
    Write-Host "║  2. 停止服務                                        ║"
    Write-Host "║  3. 狀態查看                                        ║"
