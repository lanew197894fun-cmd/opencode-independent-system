...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                                                                           ║
# ║   🦞 OpenCode 一鍵部署腳本 v1.0.0                                          ║
# ║   智慧 AI 助手部署工具 - 支援多平台多模型                                    ║
# ║                                                                           ║
# ║   GitHub: https://github.com/miaoxworld/OpenCodeInstaller                 ║
# ║   官方文檔: https://clawd.bot/docs                                         ║
# ║                                                                           ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
#
# 使用方法:
#   curl -fsSL https://raw.githubusercontent.com/miaoxworld/OpenCodeInstaller/main/install.sh | bash
#   或本地執行: chmod +x install.sh && ./install.sh
#

set -e

# ================================ TTY 偵測 ================================
# 當透過 curl | bash 執行時，stdin 是管道，需要從 /dev/tty 讀取使用者輸入
if [ -t 0 ]; then
    # stdin 是終端
    TTY_INPUT="/dev/stdin"
else
...
MIN_NODE_VERSION=22
GITHUB_REPO="miaoxworld/OpenCodeInstaller"
...
detect_os() {
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        log_info "偵測到 Linux 系統: $OS $OS_VERSION (包管理器: $PACKAGE_MANAGER)"
...
install_nodejs() {
    ...
    case "$OS" in
        ...
        arch|manjaro)
            ...
            ;;
        *)
            log_error "无法自動安裝 Node.js，請手動安裝 v$MIN_NODE_VERSION+"
            exit 1
            ;;
    esac
...
install_openclaw() {
    ...
    # 使用 npm 全局安裝
    log_info "正在从 npm 安裝 OpenCode..."
    npm install -g openclaw@$OPENCLAW_VERSION --unsafe-perm
    
    # 验证安裝
    if check_command openclaw; then
        log_info "OpenCode 安裝成功: $(openclaw --version 2>/dev/null || echo 'installed')"
        init_openclaw_config
    else
        log_error "OpenCode 安裝失敗"
        exit 1
    fi
}
...
init_openclaw_config() {
    ...
    # 設定 gateway.mode 为 local
    if check_command openclaw; then
        openclaw config set gateway.mode local 2>/dev/null || true
        log_info "Gateway 模式已設定为 local"
...
# 配置 OpenCode 使用的 AI 模型和 API Key
configure_openclaw_model() {
    log_step "配置 OpenCode AI 模型..."
    ...
    if check_command openclaw; then
        ...
        if [ -n "$openclaw_model" ]; then
            ...
            if [ $set_exit -eq 0 ]; then
                log_info "預設模型已設定为: $openclaw_model"
            else
                log_warn "模型設定可能失敗: $openclaw_model"
                echo -e "  ${GRAY}$set_result${NC}" | head -3
                
                # 尝试直接使用 config set
                log_info "尝试使用 config set 設定模型..."
...
# create_default_config 已移除 - OpenCode 使用 openclaw.json 和环境变量
...
run_onboard_wizard() {
    ...
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}           🧙 OpenCode 核心配置向导${NC}"
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    if [ -f "$env_file" ]; then
        echo -e "${YELLOW}偵測到已有配置！${NC}"
        echo ""
        
        # 顯示当前模型配置
        if check_command openclaw; then
            echo -e "${CYAN}当前 OpenCode 配置:${NC}"
        ...
        if ! confirm "是否重新配置 AI 模型提供商？" "n"; then
            ...
            if confirm "是否测试现有 API 連線？" "y"; then
                # 从 env 檔案读取配置进行测试
                source "$env_file"
                # 获取当前模型
    ...
    if [ "$skip_ai_config" = false ]; then
        setup_ai_provider
        # 先配置 OpenCode（設定环境变量和自定义 provider），然后再测试
        configure_openclaw_model
        test_api_connection
    else
        # 即使跳过配置，也可選擇测试連線
...
setup_ai_provider() {
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}  第 1 步: 選擇 AI 模型提供商${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo ""
    echo -en "${YELLOW}請選擇 AI 提供商 [1-9] (預設: 1): ${NC}"; read ai_choice < "$TTY_INPUT"
    ...
    case $ai_choice in
        1)
            ...
            echo "  5) 自定义模型名称"
            echo -en "${YELLOW}選擇模型 [1-5] (預設: 1): ${NC}"; read model_choice < "$TTY_INPUT"
        ...
        2)
            ...
            echo "選擇模型:"
            echo "  1) gpt-4o (推薦)"
            echo "  2) gpt-4o-mini (經濟)"
            echo "  3) gpt-4-turbo"
            echo "  4) 自訂模型名稱"
            echo -en "${YELLOW}選擇模型 [1-4] (預設: 1): ${NC}"; read model_choice < "$TTY_INPUT"
            case $model_choice in
                2) AI_MODEL="gpt-4o-mini" ;;
                3) AI_MODEL="gpt-4-turbo" ;;
                4) echo -en "${YELLOW}輸入模型名稱: ${NC}"; read AI_MODEL < "$TTY_INPUT" ;;
                *) AI_MODEL="gpt-4o" ;;
            esac
            ...
            if [ -n "$BASE_URL" ]; then
                ...
                echo -en "${YELLOW}選擇 API 格式 [1-2] (預設: 2): ${NC}"; read api_type_choice < "$TTY_INPUT"
        ...
        9)
            ...
            echo ""
            echo -en "${YELLOW}Ollama 地址 (預設: http://localhost:11434): ${NC}"; read BASE_URL < "$TTY_INPUT"
            BASE_URL=${BASE_URL:-"http://localhost:11434"}
            echo ""
            echo "選擇模型:"
            echo "  1) qwen2.5-coder:3b (推薦，代碼能力強)"
            echo "  2) llama3"
            echo "  3) llama3:70b"
            echo "  4) mistral"
            echo "  5) 自訂"
            ...
            case $model_choice in
                2) AI_MODEL="llama3" ;;
                3) AI_MODEL="llama3:70b" ;;
                4) AI_MODEL="mistral" ;;
                5) echo -en "${YELLOW}輸入模型名稱: ${NC}"; read AI_MODEL < "$TTY_INPUT" ;;
...
test_api_connection() {
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}  第 2 步: 测试 API 連線${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    if ! check_command openclaw; then
        echo -e "${YELLOW}OpenCode 未安裝，跳过测试${NC}"
        return 0
    fi
    ...
    while [ "$test_passed" = false ] && [ $retry_count -lt $max_retries ]; do
        ...
        if [ $exit_code -eq 0 ] && ! echo "$filtered_result" | grep -qiE "^error:|api error|401|403|Unknown model|超时|Incorrect API|authentication failed"; then
            test_passed=true
            echo -e "${GREEN}✓ OpenCode AI 测试成功！${NC}"
            echo ""
            # 顯示 AI 响应（过滤掉空行和系統日志）
            ...
            echo -e "${RED}✗ OpenCode AI 测试失敗 (退出码: $exit_code)${NC}"
            echo ""
            ...
            if [ $retry_count -lt $max_retries ]; then
                ...
                if echo "$filtered_result" | grep -qi "Unknown model"; then
                    ...
                    echo -e "${YELLOW}提示: API 可能没有正确响应，請检查 API 地址和模型名称${NC}"
                fi
                echo ""
...
setup_identity() {
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}  第 3 步: 設定身份資訊${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    
    echo -en "${YELLOW}给你的 AI 助手起个名字 (預設: Clawd): ${NC}"; read BOT_NAME < "$TTY_INPUT"
    BOT_NAME=${BOT_NAME:-"Clawd"}
    
    echo -en "${YELLOW}AI 如何称呼你 (預設: 主人): ${NC}"; read USER_NAME < "$TTY_INPUT"
    USER_NAME=${USER_NAME:-"主人"}
    
    echo -en "${YELLOW}你的时区 (預設: Asia/Shanghai): ${NC}"; read TIMEZONE < "$TTY_INPUT"
...
# 啟動 OpenCode Gateway 服务
start_openclaw_service() {
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}           🚀 啟動 OpenCode 服务${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    # 使用端口偵測判断是否已有服务在运行（更可靠）
    local existing_pid=$(lsof -ti :18789 2>/dev/null | head -1)
    if [ -n "$existing_pid" ]; then
        log_warn "OpenCode Gateway 已在运行 (PID: $existing_pid)"
        echo ""
    ...
    if [ -n "$gateway_pid" ]; then
        echo ""
        echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${GREEN}           ✓ OpenCode Gateway 已啟動！(PID: $gateway_pid)${NC}"
        echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo ""
        ...
        echo ""
        log_info "OpenCode 现在可以接收消息了！"
    else
        log_error "Gateway 啟動失敗"
        echo ""
        echo -e "${YELLOW}請查看日志: tail -f /tmp/openclaw-gateway.log${NC}"
        echo -e "${YELLOW}或手動啟動: source ~/.openclaw/env && openclaw gateway${NC}"
    fi
}
...
run_config_menu() {
    ...
    if [ "$has_local_menu" = true ]; then
        log_info "偵測到本地配置菜单: $menu_script"
        echo ""
...
main() {
    ...
    echo -e "${YELLOW}⚠️  警告: OpenCode 需要完全的计算机权限${NC}"
    ...
    # 询问是否啟動服务
    if confirm "是否现在啟動 OpenCode 服务？" "y"; then
        start_openclaw_service
    else
        echo ""
    ...
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}           🖥️ 推薦：OpenCode Manager 桌面版${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo -e "${WHITE}如果你更喜欢图形界面，推薦下载 OpenCode Manager 桌面应用：${NC}"
    echo ""
    ...
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo -e "${GRAY}配置菜单支援: 渠道配置、身份設定、安全配置、服务管理等${NC}"
    echo ""
    ...
    echo ""
    echo -e "${GREEN}🦞 OpenCode 安裝完成！祝你使用愉快！${NC}"
    echo ""
}
