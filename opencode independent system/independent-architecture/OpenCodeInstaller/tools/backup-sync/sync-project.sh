...
list_projects() {
    ...
    local projects=(
        "$HOME/openclaw-manager:OpenCode Manager 主程式"
        "$HOME/openclaw-manager/openclaw:OpenCode 龍蝦系統"
        "$HOME/openclaw-manager/opencode independent system:OpenCode 開發環境"
        "$HOME/.opencode:OpenCode 配置"
        "$HOME/.openclaw:OpenCode 配置"
    )
...
sync_opencode() {
    ...
    TARGET_DIR="$HOME/OpenCode_Backups/opencode_dev"
    do_sync
}

sync_openclaw_manager() {
    SOURCE_PATH="$HOME/openclaw-manager"
    TARGET_DIR="$HOME/OpenCode_Backups/openclaw_manager"
    do_sync
}
